...
/**
 * Request real SMS OTP dispatch via backend Twilio gateway.
 */
export async function sendPhoneOtpApi(phone: string): Promise<OtpResponse> {
  const response = await fetch(`${API_BASE_URL}/api/otp/send-phone`, {
    method: 'POST',
  ...
  if (!response.ok) {
    let errorMsg = `Failed to send SMS OTP (${response.status})`;
    try {
...
/**
 * Verify phone OTP against backend verification store.
 */
export async function verifyPhoneOtpApi(phone: string, otp: string): Promise<OtpResponse> {
  const response = await fetch(`${API_BASE_URL}/api/otp/verify-phone`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
/**
 * Request Email OTP dispatch via backend mail gateway.
 */
export async function sendEmailOtpApi(email: string): Promise<OtpResponse> {
  const response = await fetch(`${API_BASE_URL}/api/otp/send-email`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!response.ok) {
    let errorMsg = `Failed to send Email verification (${response.status})`;
    try {
      const err = await response.json();
      if (err.detail) errorMsg = err.detail;
    } catch {}
...
/**
 * Verify email OTP against backend verification store.
 */
export async function verifyEmailOtpApi(email: string, otp: string): Promise<OtpResponse> {
  const response = await fetch(`${API_BASE_URL}/api/otp/verify-email`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!response.ok) {
    let errorMsg = 'Invalid email verification code.';
    try {
      const err = await response.json();
      if (err.detail) errorMsg = err.detail;
    } catch {}
